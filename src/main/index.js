...
ipcMain.on('take-screenshot', async (event) => {
  try {
    ...
    const sourcesWithThumbnails = await desktopCapturer.getSources(screenshotOptions);

    const source = sourcesWithThumbnails.find((s) => s.name === 'Entire screen' || s.name === 'Screen 1');

    if (source) {
      const timestamp = new Date().toISOString().replace(/:/g, '-');
      const screenshotPath = app.getPath('desktop') + `/screenshot_${timestamp}.png`;
